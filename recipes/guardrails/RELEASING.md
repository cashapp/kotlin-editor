...
1. Publish the snapshot to Maven Central by invoking the `publish-gradle-guard` action on github.
   1. This action has a required input, `core_libs_version`, which must be the non-snapshot release of the "core libs"
      (that is, everything other than `gradle-guard` in this repo).
